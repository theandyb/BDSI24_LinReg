...
Question: in general, does house price tend to increase with the size of living space? Answer:
...
Q: What is our fitted model: A: price = ??? + ??? x sqft_living

Q: Does the slope have a meaningful interpretation here? Hint: When would a house be predicted to have a price of -43580.743? Does this make sense? A:

Q: Interpret the coefficient for sqft_living. Is a higher sqft_living value associated with a higher price? A:

Finally, we can use the `predict()` function to get fitted values for `price` for given `sqft_living` values:

```{r}
predict(model1, newdata = data.frame("sqft_living" = 2080), interval="predict")
```

Q: According to our model, what is the predicted price of a home with 2080 sq ft of living space? A:
...
Q: Visually, does it look like there is a difference in the distributions of prices between waterfront (1) and non-waterfront (0) properties? A:
...
Q: What is our fitted model? A: Price = ? + ? x waterfront

Q: How do we interpret the intercept coefficient? Hint: What price would our model predict for a home not on a waterfront? (waterfront = 0) A:

Q: How do we interpret the waterfront coefficient? A: On average, a house on the waterfront is \_\_\_\_\_\_\_ more/less expensive than one on a non-waterfront property.

Q: How does the t value for the waterfrontY coefficient compare to the t statistic from the t test we ran above? A:
...
Q: What is the equation for our fitted model? A: price. = ??? + ??? x sqft_living + ??? x waterfront

Q: How does the coefficient for `sqft_living` in model3 compare to what we observed in model1? A:

Q: How does the coefficient for `waterfrontY` in model3 compare to what we observed in model2? A:

Q: Between models 1 - 3, which has the highest Adjusted R-squared value? A:

## Exercise 5

We're going to extend our previous model to allow for an interaction between `sqft_living` and `waterfront`. This will allow for the relationship between `sqft_living` and `price` to differ between the waterfront and non-waterfront properties.

Fit model four with the interaction:

```{r}
model4 <- lm(price ~ sqft_living*waterfront, data = df_home)
